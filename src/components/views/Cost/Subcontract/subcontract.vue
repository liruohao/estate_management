  ...
  <div class="bgcolor">
    <div>
      <Breadcrumb class="bread">
        ...
        <BreadcrumbItem>分包管理</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <div class="operation">
      <div>
        <table width="100%">
          <tr>
            <td>
              <span class="fontFloat">单据号:</span>
              <Input v-model="select.bill" style="width: 80%" />
            </td>
            <td class="tabfloat">
              <span class="fontFloat">单位:</span>
              <Input v-model="select.company" style="width: 80%" />
            </td>
            <td  class="tabfloat">
              <span class="fontFloat">分包名称:</span>
              <Input v-model="select.contractname" style="width: 75%" />
            </td>
          </tr>
          <tr>
            <td>
              <span class="fontFloat">我方负责人:</span>
              <Input v-model="select.owncharge"  style="width: 73%"></Input>
            </td>
            <td class="tabfloat">
              <span class="fontFloat" style="padding-left: 3%">合作方负责人:</span>
              <Input v-model="select.othercharge  " style="width: 62%" />
            </td>
            <td class="tabbut">
    ...
    <Modal v-model="editorshow"  width="60%"
      ...
      <Form :model="formItem" :label-width="80">
        <FormItem label="单据号" class="titfloat">
          <Input id="rubric" value="${this.formItem.bill}"  v-model="formItem.bill" placeholder="请输入单据号" :disabled="this.falg ===3"></Input>
        </FormItem>
  ...
  export default {
    ...
    methods:{
      ...
      handleListApproveHistory(){
        //通过JSON引入数据
        this.$http.get('/api/subcontract').then((response)=>{
          response=response.body;
          // // 保存取到的所有数据
          this.ajaxHistoryData = response.data;
          this.dataCount = response.data.length;
          // 初始化显示，小于每页显示条数，全显，大于每页显示条数，取前每页条数显示
          if(response.data.length < this.pcontractnameSize){
            this.historyData = this.ajaxHistoryData;
          }else{
            this.historyData = this.ajaxHistoryData.slice(0,this.pcontractnameSize);
          }
        });
